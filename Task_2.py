...
class ConsoleInterface(UserInterface):
    ...
    def display_contacts(self, contacts):
        if not contacts:
            self.print_message("No contacts found.")
        else:
            for contact in contacts:
                self.print_message(str(contact))
    ...
    def display_help(self):
        ...
        - add-birthday <name> <birthday>: Add or change the birthday of a contact.
...
def main(interface):
    book = load_data()
    interface.print_message("Welcome to the assistant bot!")
    while True:
        user_input = interface.input_command()
        command, args = parse_input(user_input)

        if command in ["close", "exit"]:
            save_data(book)  
            interface.print_message("Good bye!")
            break

        elif command == "hello":
            interface.print_message("How can I help you?")

        elif command == "add":
            interface.print_message(add_contact(args, book))

        elif command == "change":
            interface.print_message(change_contact(args, book))

        elif command == "phone":
            interface.print_message(get_phone(args, book))

        elif command == "all":
            interface.display_contacts(book.values())

        elif command == "add-birthday":
            interface.print_message(add_birthday(args, book))

        elif command == "show-birthday":
            interface.print_message(show_birthday(args, book))

        elif command == "birthdays":
            interface.display_contacts(book.get_upcoming_birthdays())

        elif command == "help":
            interface.display_help()

        else:
            interface.print_message("Invalid command.")
